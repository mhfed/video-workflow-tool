...
Draw Reveal accepts project-local `.png`, `.jpg`, or `.jpeg` artwork through `scene.artwork`. It generates a serpentine normalized path by default; `drawReveal.path` can replace it with explicit `[x, y]` points from 0 to 1. The existing hand PNG is reused by default, while `drawReveal.handAsset` may point to another project-local image. It uses the same narration lifecycle and supports the existing `backgroundMusic` field plus renderer-local subtitle/music settings.
