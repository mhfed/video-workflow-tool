...
export function config() {
  ...
  return {
    ...
    ffmpegBin: env.FFMPEG_BIN || 'ffmpeg', ffprobeBin: env.FFPROBE_BIN || 'ffprobe', pythonBin: env.PYTHON_BIN || 'python3',
    ...
    sceneTargetSec: Number(env.SCENE_TARGET_SEC || 12), sceneMinSec: Number(env.SCENE_MIN_SEC || 6), sceneMaxSec: Number(env.SCENE_MAX_SEC || 18), wordsPerMinute: Number(env.WORDS_PER_MINUTE || 150),
    width: Number(env.VIDEO_WIDTH || 1920), height: Number(env.VIDEO_HEIGHT || 1080), fps: Number(env.VIDEO_FPS || 30),
    webHost: env.WEB_HOST || '127.0.0.1', webPort: Number(env.WEB_PORT || 4173)
  };
}
