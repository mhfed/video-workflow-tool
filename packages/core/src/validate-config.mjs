...
export function validateConfig(cfg){
  ...
  if(!TEXT_PROVIDERS.has(cfg.textProvider))errors.push(`TEXT_PROVIDER must be one of: ${[...TEXT_PROVIDERS].join(', ')}`);
  if(!IMAGE_PROVIDERS.has(cfg.imageProvider))errors.push(`IMAGE_PROVIDER must be one of: ${[...IMAGE_PROVIDERS].join(', ')}`);
  ...
  if(cfg.codexTimeoutMs!==undefined&&(!Number.isFinite(cfg.codexTimeoutMs)||cfg.codexTimeoutMs<=0))errors.push('CODEX_TIMEOUT_MS must be greater than 0.');
