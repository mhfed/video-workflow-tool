...
export default function App(){
  ...
  const loadRetention=async()=>api(`/api/projects/${encodeURIComponent(current.id)}/retention`);
  const mutateRetention=async(path,options)=>{setBusy(true);setError('');try{const result=await api(`/api/projects/${encodeURIComponent(current.id)}/retention${path}`,options);setCurrent(result.project);await refreshProjects();return result;}catch(cause){setError(cause.message);throw cause;}finally{setBusy(false);}};
  const planRetention=async()=>mutateRetention('/plan',{method:'POST'});
  const saveRetention=async(plan)=>mutateRetention('',{method:'PATCH',body:JSON.stringify(plan)});
  const selectRetentionHook=async(variantId)=>mutateRetention('/hooks/select',{method:'POST',body:JSON.stringify({variantId})});
  const mutatePackaging=async(path,body={})=>{setBusy(true);setError('');try{const result=await api(`/api/projects/${encodeURIComponent(current.id)}/packaging/${path}`,{method:'POST',body:JSON.stringify(body)});setCurrent(result.project);await refreshProjects();return result;}catch(cause){setError(cause.message);throw cause;}finally{setBusy(false);}};
  const planPackaging=async()=>mutatePackaging('plan');
  const selectPackaging=async(variantId)=>mutatePackaging('select',{variantId});
  const applyPackaging=async(variantId)=>mutatePackaging('apply',{variantId,applyTitle:true,applyHook:true});
